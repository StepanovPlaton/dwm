...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x, statusvertpadbar, w, bh - 2*statusvertpadbar, 0, text, 0);
...
drawbar(Monitor *m)
{
	...
	x = 0;
	drw_text(drw, 0, 0, 5, bh, lrpad / 2, "", 0);
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNorm]);
		drw_text(drw, x + 5, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
		if (occ & 1 << i)
			drw_rect(drw, x +5 + boxs, boxs, boxw, boxw,
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x, systrayvertpad, i->w, i->h - systrayvertpad*2);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
